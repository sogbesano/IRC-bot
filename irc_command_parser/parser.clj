...
(defn parse-nick
  ...
  [src]
  (if (includes? src "!") (subs src (+ (index-of src ":") 1) (index-of src "!")) nil))
...
(defn parse-host
  ...
  [rx-irc-cmd]
  (let [host-and-tail (if (re-find #"@(\w+)" rx-irc-cmd) (subs rx-irc-cmd (+ (index-of rx-irc-cmd "@") 1)) " ")]
    (if (includes? host-and-tail " ") (subs host-and-tail 0 (index-of host-and-tail " ")) host-and-tail)))

(defn parse-cmd-name
  "Parses IRC command name data from rx-irc-cmd"
  [rx-irc-cmd]
  (let [matcher (re-matcher #"(\s+)\w+|\d+(\s+)" rx-irc-cmd)]
    (if (re-find matcher) (trim (join "" (re-groups matcher))) nil)))

(defn parse-cmd-args
  "Parses IRC command args data from rx-irc-cmd"
  [rx-irc-cmd]
  (let [cmd-matcher (re-matcher #"(\s+)\w+|\d+(\s+)" rx-irc-cmd) cmd-args-matcher (re-matcher #" :(.+)" rx-irc-cmd)]
...
(defn parse-final-arg
  ...
  [rx-irc-cmd]
  (let [final-arg-matcher (re-matcher #"(\s+):(.+)" rx-irc-cmd)]
...
(defn parse-irc-cmd
  ...
  [rx-irc-cmd]
  (let [src (parse-source rx-irc-cmd)
        nick (parse-nick src)
